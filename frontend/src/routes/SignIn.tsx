...
export function SignIn() {
  ...
  return (
    <div className="min-h-dvh bg-paper">
      ...
      {/* Sticky, translucent header: wordmark, section anchors, sign-in shortcut. */}
      <header className="sticky top-0 z-40 border-b border-[var(--glass-edge)] bg-[var(--glass-surface)] backdrop-blur-[var(--glass-blur)]">
          ...
          <nav aria-label="Landing page sections" className="hidden items-center gap-1 md:flex">
            {NAV_LINKS.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="rounded-(--radius-control) px-3 py-2 text-sm text-ink-2 transition-colors duration-150 hover:bg-sunken hover:text-ink"
              >
                {link.label}
              </a>
            ))}
