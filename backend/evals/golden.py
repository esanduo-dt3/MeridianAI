...
VALID_KINDS = {"single_passage", "multi_hop", "cross_doc", "table", "unanswerable"}
...
class GoldenQuestion:
    ...
    def scored_for_gate(self) -> bool:
        """G1 counts citations, which an unanswerable question does not have."""
...
def _one(raw: dict, path: Path, number: int) -> GoldenQuestion:
    ...
    return GoldenQuestion(
        ...
        profile=profile,
        kind=kind,
        document=document.strip() if isinstance(document, str) else None,
...
def _normalise(text: str) -> tuple[str, list[int]]:
    ...
    for index, character in enumerate(text):
        ...
        folded.append(_fold_char(character))
        offsets.append(index)
